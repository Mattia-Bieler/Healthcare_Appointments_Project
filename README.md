...
## Visualisations
At the beginning of the Jupyter notebook, custom configurations were set up to ensure consistent and accessible visualisations, prioritising clarity by making plot legends, labels, and titles bold, as well as setting their font size. A custom colourblind-friendly palette was developed by incorporating official NHS colours, ensuring a visually distinct and accessible combination. The selected colours provide strong contrast and differentiation while aligning with NHS branding. Seaborn’s colourblind palette was taken into consideration during the design process to enhance accessibility and maintain clear distinctions between hues.

The default graph colour for the project is NHS Blue, with red used to highlight thresholds and the estimated cost of unattended appointments for clear visual emphasis. The custom colourblind-friendly palette was applied to graphs displaying appointment data over time across different categories. For regional appointment data over time, an official NHS regional colour palette was used to align with NHS standards.
